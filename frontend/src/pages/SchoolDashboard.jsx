...
import { BarChart3, GraduationCap, Users, Sparkles, ArrowRight, Building2, UserPlus, Copy, Loader2 } from "lucide-react";
...
export default function SchoolDashboard({ variant = "counselor" }) {
  ...
  // Counselor Add-Student state
  const [showStudentForm, setShowStudentForm] = useState(false);
  const [stForm, setStForm] = useState({ name: "", student_email: "", grade: "", parent_email: "" });
  ...
  const createStudent = async (e) => {
    ...
    setCreating(true);
    try {
      const payload = Object.fromEntries(Object.entries(stForm).filter(([, v]) => v !== ""));
      const { data } = await api.post("/counselor/students", payload);
      setCreatedStudent(data);
      setStForm({ name: "", student_email: "", grade: "", parent_email: "" });
      toast.success("Student added");
    } catch (e) {
      toast.error(formatApiErrorDetail(e.response?.data?.detail) || "Failed to create student");
    } finally { setCreating(false); }
  };

  const copyCreds = () => {
    if (!createdStudent) return;
    const s = createdStudent.student;
    let txt = `Student: ${s.email} | ${s.temp_password}`;
    if (createdStudent.parent?.temp_password) txt += `\nParent: ${createdStudent.parent.email} | ${createdStudent.parent.temp_password}`;
...
.alignment_distribution.needs_reflection || 0} color="bg-[#FFDDBF]" />
        </div>
